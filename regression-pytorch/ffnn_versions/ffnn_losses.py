...
propertyRangeList = ["Mean Squared Error", "Mean Absolute Error", "Smooth L1 Loss", "Huber Loss", "Log-Cosh Loss", "Quantile Loss", "Mean Absolute Percentage Error"]
propertyAppend = []
for changingProperty in propertyRangeList:
    ## instantiate the model and move it to GPU if available
    model = FFNN().to(device)
    if run_mode == 1 or run_mode == 2:
        if changingProperty == "Mean Squared Error":
            criterion = nn.MSELoss()
        elif changingProperty == "Mean Absolute Error":
            criterion = nn.L1Loss()
        elif changingProperty == "Smooth L1 Loss":
            criterion = nn.SmoothL1Loss()
        elif changingProperty == "Huber Loss":
            criterion = nn.HuberLoss()
        elif changingProperty == "Log-Cosh Loss":
            criterion = log_cosh_loss
        elif changingProperty == "Quantile Loss":
            criterion = quantile_loss(0.9)
        elif changingProperty == "Mean Absolute Percentage Error":
            criterion = mape_loss
...
with open(r"EpochsData\losses_data.txt", "w") as f:
