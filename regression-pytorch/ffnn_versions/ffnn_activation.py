...
propertyRangeList = ["Tanh", "ReLU", "LeakyReLU", "Sigmoid"]
propertyAppend = []
for changingProperty in propertyRangeList:
    ## instantiate the model and move it to GPU if available
    if changingProperty == "Tanh":
        model = FFNN().to(device)
    elif changingProperty == "ReLU":
        model = FFNN2().to(device)
    elif changingProperty == "LeakyReLU":
        model = FFNN4().to(device)
    elif changingProperty == "Sigmoid":
        model = FFNN3().to(device)
...
with open(r"EpochsData\activation_functions_data.txt", "w") as f:
